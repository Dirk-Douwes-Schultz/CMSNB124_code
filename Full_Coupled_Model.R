#code for the Full Coupled Model from Section 4 of the main text
  ...
  for(i in 1:N){
    ...
    for(t in 2:mT){
      ...
      logit(p12[i,t]) <- alpha[1]+alpha[5]*(standard_cap[i]-mean(standard_cap[1:N]))+
        alpha[10]*(mobility_matrix[i,t-1]-mmobility_matrix)+alpha[15]*prev_nei_epi_sum[i,t]
      lp21op22[i,t] <- alpha[2]+alpha[6]*(standard_cap[i]-mean(standard_cap[1:N]))+
        alpha[11]*(mobility_matrix[i,t-1]-mmobility_matrix)+alpha[16]*prev_nei_epi_sum[i,t]
      ...
      lp23op22[i,t] <- alpha[3]+alpha[7]*(mobility_matrix[i,t-1]-mmobility_matrix)+
        ...
        alpha[13]*prev_nei_epi_sum[i,t]+alpha[17]*new_variant[t]
...
for(i in 1:N){
  #initial state is set to 2 here to make testing the samplers easier
  S_init[i,1] <- 2
  for(t in 2:mT){
...
model$getLogProb()
model$calculate()
#if it worked properly it should sample new values of the hidden chain
#and getLogProb and calculate should return the same value
...
plot(samples[,c("alpha[12]","alpha[13]","alpha[7]","alpha[14]")])
plot(samples[,c("alpha[15]","alpha[16]","alpha[17]")])
